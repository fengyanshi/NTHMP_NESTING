...
% input
foutput='/Volumes/DISK_2020_5/NTHMP_FL_PM_BR/Results/LSB345_GridA/output/';
fgrid='/Users/fengyanshi/WORK/work/NTHMP_2022/make_stations/';
fcur='/Users/fengyanshi/WORK/work/NTHMP_2022/make_coupling_files/';
coupling_info=load([fgrid 'station_G_to_FL.txt']);
num_blocks=coupling_info(end,3)/4;

iscale=[10];  % spacial refine scale for all blocks
itscale = [1]; % coarsen time scale, skip x output time steps

starttime  = [28000];   % in seconds, adjust to truncate time series
stoptime=starttime+18000.0; % 5 hours

%stoptime   = [11000 11000];% Inf; % in sec. indicate Inf to include all.

coupling_filename={'coupling_LSB345_FL.txt'}
logfile='log_LSB345_AB.txt';
